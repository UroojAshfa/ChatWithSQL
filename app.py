...
selected_opt = st.sidebar.radio(
    label="Choose the DB which you want to chat with:", options=radio_opt
)
...
if radio_opt.index(selected_opt) == 1 and not all(
...
llm = ChatGroq(
    ...
    model_name="Gemma2-9b-It",  # Use valid Groq model
    streaming=True,
)
...
def configure_db(db_uri, mysql_host=None, mysql_user=None, mysql_password=None, mysql_db=None):
    if db_uri == LOCALDB:
        ...
        return SQLDatabase(
            create_engine(
                f"mysql+mysqlconnector://{mysql_user}:{mysql_password}@{mysql_host}:3306/{mysql_db}?charset=utf8mb4&auth_plugin=mysql_native_password"
            )
        )
